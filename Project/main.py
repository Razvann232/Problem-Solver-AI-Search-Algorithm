...
font = pygame.font.Font("8bitlim.ttf", FONT_SIZE)
    ...
    if x + BUTTON_WIDTH > mousePosition[0] > x and y + BUTTON_HEIGHT > mousePosition[1] > y:
        ...
        if mouseClicked[0] == 1 and action is not None:
            if action == "goToMenu":
                ...
                problema = algorithms.cannibalsProblem(difficulty)

                if algorithm == "DFS":
                    solution = algorithms.depthFirstSearch(problema)
                elif algorithm == "BFS":
                    solution = algorithms.breadthFirstSearch(problema)
                elif algorithm == "UCS":
                    solution = algorithms.uniformCostSearch(problema)
                elif algorithm == "AStar":
                    solution = algorithms.aStarSearch(problema)
